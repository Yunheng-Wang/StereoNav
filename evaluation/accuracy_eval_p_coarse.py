...
class VLNEvaluator:
    def __init__(
        ...
        self.coarse_radius = args.coarse_radius
    ...
    def sample_coarse_goal(self, goal_position):
        """Sample a random point within coarse_radius of the true goal on the XZ plane.
        Returns (sampled_position, distance_to_true_goal)."""
        angle = random.uniform(0, 2 * np.pi)
        r = np.sqrt(random.uniform(0, 1)) * self.coarse_radius
        offset = np.array([r * np.cos(angle), 0.0, r * np.sin(angle)])
        sampled_position = goal_position + offset
        distance_to_true = np.linalg.norm(offset)
        return sampled_position, distance_to_true
    ...
    def eval_action(self, idx) -> None:
        ...
        for scene in sorted(scene_episode_dict.keys()):
            ...
            for episode in episodes[idx::self.env_num]:
                ...
                # Sample a coarse goal near the true goal within coarse_radius
                coarse_goal_position, coarse_goal_offset = self.sample_coarse_goal(goal_position)
                ...
                while not env.episode_over:
                    ...
                    # Project coarse goal onto stereo views and draw as red dot
                    left_point, right_point = self.project_goal_to_stereo(env, coarse_goal_position)
                ...
                print(f"scene_episode {scene_id}_{episode_id} success: {metrics['success']}, spl: {metrics['spl']}, os: {metrics['oracle_success']}, ne: {metrics['distance_to_goal']}, tl: {trajectory_length:.2f}, coarse_offset: {coarse_goal_offset:.2f}")
                ...
                result = {
                    ...
                    "coarse_goal_offset": round(coarse_goal_offset, 4),
                }
...
def eval():
    ...
    parser.add_argument("--coarse_radius", type=float, default=3.0, help="coarse goal sampling radius in meters")
