...
class VLNEvaluator:
    def __init__(
        ...
        self.light_perturbation = args.light_perturbation
        self.use_postprocess_light = False  # Flag for fallback method
    ...
    def setup_light_perturbation(self, env):
        """Setup light perturbation in Habitat simulator using native rendering.
        Modifies the scene's light setup to adjust brightness.
        Args:
            env: Habitat environment
        """
        if self.light_perturbation == 0:
            return

        # Habitat native light control has limited effect due to ambient light
        # Use post-processing instead for more reliable results
        print(f"[Light] Using post-processing method with perturbation factor: {self.light_perturbation:.2f}")
        self.use_postprocess_light = True

    def apply_light_perturbation_postprocess(self, rgb_image):
        """Fallback: Apply light perturbation via post-processing.
        Args:
            rgb_image: numpy array [H, W, 3] with values 0-255
        Returns:
            perturbed RGB image with same shape and range
        """
        if self.light_perturbation == 0 or not self.use_postprocess_light:
            return rgb_image
        ...
        if self.light_perturbation > 0:
            ...
            factor = self.light_perturbation
            img_float = img_float + (255.0 - img_float) * factor
        else:
            # Darken: blend with black
            factor = -self.light_perturbation
            img_float = img_float * (1.0 - factor)

        # Clip to valid range and convert back
        img_perturbed = np.clip(img_float, 0, 255).astype(np.uint8)
        return img_perturbed

    def preprocess_rgb(self, rgb_image):
        """Preprocess RGB image to [1, 3, 448, 448] tensor with values 0~255"""
        # Apply post-process light perturbation if native method failed
        rgb_image = self.apply_light_perturbation_postprocess(rgb_image)
    ...
    def eval_action(self, idx) -> None:
        ...
        # Setup light perturbation once for the entire evaluation
        # This modifies the simulator's light setup using native rendering
        self.setup_light_perturbation(env)
...
def eval():
    ...
    parser.add_argument("--light_perturbation", type=float, default=0.0, help="light perturbation factor: 0=default, positive=brighter, negative=darker (range: -1 to 1)")
