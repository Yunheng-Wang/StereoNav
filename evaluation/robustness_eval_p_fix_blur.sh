#!/bin/bash
# Distributed robustness evaluation script for StereoVLN with motion blur perturbation.
...
# Usage: bash evaluation/robustness_eval_p_fix_blur.sh
...
# Motion blur strength (0 = no blur, higher = stronger blur)
MOTION_BLUR_STRENGTH=8.0

echo "Server URLs: ${SERVER_URLS}"
echo "Eval split: ${EVAL_SPLIT}"
echo "Output path: ${OUTPUT_PATH}"

CMD="torchrun --nproc_per_node=8 --master_port=$MASTER_PORT evaluation/robustness_eval_p_fix_blur.py \
    --server_urls $SERVER_URLS \
