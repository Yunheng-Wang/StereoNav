...
class VLNEvaluator:
    def __init__(
        ...
        self.height_offset = args.height_offset

        # Configure stereo camera sensors with height offset
        # Base height (1.25m) + height offset
        base_height = 1.25
        camera_height = base_height + self.height_offset

        with habitat.config.read_write(self.config):
            OmegaConf.set_struct(self.config, False)
            self.config.habitat.dataset.split = self.split

            # Left RGB camera: offset 5cm to the left at adjusted height
            self.config.habitat.simulator.agents.main_agent.sim_sensors.rgb_sensor_left = OmegaConf.create({
                ...
                "height": 448,
                "hfov": 79,
                "position": [-0.05, camera_height, 0.0],  # left eye, 5cm left of center
                "orientation": [0.0, 0.0, 0.0]
            })
            # Right RGB camera: offset 5cm to the right at adjusted height
            self.config.habitat.simulator.agents.main_agent.sim_sensors.rgb_sensor_right = OmegaConf.create({
                ...
                "height": 448,
                "hfov": 79,
                "position": [0.05, camera_height, 0.0],  # right eye, 5cm right of center
                "orientation": [0.0, 0.0, 0.0]
            })
        ...
        # Camera offsets include the height offset applied to both eyes
        self.camera_offset_left = np.array([-0.05, camera_height, 0.0])
        self.camera_offset_right = np.array([0.05, camera_height, 0.0])
    ...
    def project_goal_to_stereo(self, env, goal_position):
        ...
        # Offset goal to camera height (including height_offset) to match training data projection
        camera_height = 1.25 + self.height_offset
        goal_camera_center = goal_position + np.array([0.0, camera_height, 0.0])
...
def eval():
    ...
    parser.add_argument("--height_offset", type=float, default=0.0, help="camera height offset in meters (e.g., 0.6 or -0.6)")
