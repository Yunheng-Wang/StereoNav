"""Distributed evaluation script for StereoVLN with exact action prediction.

Each torchrun worker handles a subset of episodes (round-robin by rank) and
communicates with a dedicated inference server via HTTP.

Metrics reported: Success Rate, SPL, Oracle Success, Navigation Error,
                  Trajectory Length, nDTW.

Run via: bash evaluation/accuracy_eval_p_exact.sh
"""
import sys
import os
import re
...
from data.utils import rxr_dataset
...
class VLNEvaluator:
    ...
    def eval_action(self, idx) -> None:
        ...
        for episode in env.episodes:
            if not getattr(episode.instruction, 'language', 'en').startswith('en'):
                continue
            if episode.scene_id not in scene_episode_dict:
                scene_episode_dict[episode.scene_id] = []
            scene_episode_dict[episode.scene_id].append(episode)

        sucs, spls, oss, ones, tls, ndtws = [], [], [], [], [], []
        done_res = []
        ...
        for scene in sorted(scene_episode_dict.keys()):
            ...
            for episode in episodes[idx::self.env_num]:
                ...
                # nDTW tracking
                agent_path = [env.sim.get_agent_state().position.tolist()]
                ...
                while not env.episode_over:
                    ...
                    agent_path.append(current_position.tolist())
                ...
                if self.save_video:
                    try:
                        ...
                        print(f"[Warning] Failed to save video for {scene_id}_{episode_id}: {e}")
                vis_frames.clear()

                # Compute nDTW
                ref_path = episode.reference_path if hasattr(episode, 'reference_path') else None
                if ref_path is not None and len(ref_path) > 0:
                    ref = np.array(ref_path)
                    agent = np.array(agent_path)
                    n, m = len(agent), len(ref)
                    dtw = np.full((n + 1, m + 1), np.inf)
                    dtw[0, 0] = 0.0
                    for i in range(1, n + 1):
                        for j in range(1, m + 1):
                            cost = np.linalg.norm(agent[i-1] - ref[j-1])
                            dtw[i, j] = cost + min(dtw[i-1, j], dtw[i, j-1], dtw[i-1, j-1])
                    ndtw = float(np.exp(-dtw[n, m] / (m * 3.0)))
                else:
                    ndtw = 0.0
                ...
                print(f"scene_episode {scene_id}_{episode_id} success: {metrics['success']}, spl: {metrics['spl']}, os: {metrics['oracle_success']}, ne: {metrics['distance_to_goal']}, tl: {trajectory_length:.2f}, ndtw: {ndtw:.4f}")
        ...
        env.close()
        return torch.tensor(sucs).to(self.device), torch.tensor(spls).to(self.device), torch.tensor(oss).to(self.device), torch.tensor(ones).to(self.device), torch.tensor(tls).to(self.device), torch.tensor(ndtws).to(self.device), torch.tensor(len(sucs)).to(self.device)
...
def evaluate(model, args):
    ...
    sucs, spls, oss, ones, tls, ndtws, ep_num = evaluator.eval_action(get_rank())
