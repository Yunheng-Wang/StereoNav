...
def learning():
    ...
    config = OmegaConf.load('train_stage2.yaml')
    ...
    resume_ckpt = config.main.get("resume_checkpoint", "")
    if resume_ckpt:
        ...
        if rank == 0:
            logging.info(f"Loaded checkpoint from {resume_ckpt}")
            if missing:
...
if __name__ == "__main__":
    ...
    # CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 accelerate launch --multi_gpu --num_processes 8 --num_machines 1 --mixed_precision fp16 --main_process_port 29555 --dynamo_backend no train_stage2.py
