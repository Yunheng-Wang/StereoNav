...
class VLNEvaluator:
    def __init__(
        ...
        self.motion_blur_strength = args.motion_blur_strength
    ...
    def apply_motion_blur(self, image, blur_radius):
        """Apply motion blur to PIL image.
        Args:
            image: PIL Image
            blur_radius: blur strength (0 = no blur)
        Returns:
    ...
    def preprocess_rgb(self, rgb_image, blur_radius=None):
        """Preprocess RGB image to [1, 3, 448, 448] tensor with values 0~255
        Args:
            rgb_image: numpy array
            blur_radius: optional blur radius for motion blur simulation
        """
        image = Image.fromarray(rgb_image).convert('RGB')

        # Apply motion blur before resizing if specified
        if blur_radius is not None and blur_radius > 0:
            image = self.apply_motion_blur(image, blur_radius)
    ...
    def get_stereo_images(self, observations):
        ...
        Applies same motion blur to both left and right images.
        """
        rgb_left = observations["rgb_left"]
        rgb_right = observations["rgb_right"]

        blur_radius = self.motion_blur_strength

        left_frame = self.preprocess_rgb(rgb_left, blur_radius=blur_radius)
        right_frame = self.preprocess_rgb(rgb_right, blur_radius=blur_radius)
...
def eval():
    ...
    parser.add_argument("--motion_blur_strength", type=float, default=0.0, help="motion blur strength (0=no blur, larger=more blur)")
