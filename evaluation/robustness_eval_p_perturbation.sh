#!/bin/bash
# Distributed robustness evaluation script for StereoVLN with lighting perturbation.
...
# Usage: bash evaluation/robustness_eval_p_perturbation.sh
...
# Lighting perturbation factor: 0=default, positive=brighter, negative=darker (range: -1 to 1)
LIGHT_PERTURBATION=0.4

echo "Server URLs: ${SERVER_URLS}"
echo "Eval split: ${EVAL_SPLIT}"
echo "Output path: ${OUTPUT_PATH}"
echo "Light perturbation: ${LIGHT_PERTURBATION}"

CMD="torchrun --nproc_per_node=8 --master_port=$MASTER_PORT evaluation/robustness_eval_p_perturbation.py \
    --server_urls $SERVER_URLS \
    ...
    --light_perturbation $LIGHT_PERTURBATION"
